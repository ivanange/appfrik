...
function init ($duration = 6000, $position = 0 ) {
    $direction = "";
    $previousPosition = 0;
    $interval=0;
    ...
    function slide ($direction, $position) {
        ...
        $position = ( $direction == "<" ?  --$position : $direction == ">" ?  ++$position : $position ) % $length ;
    ...
    for($i=0; $i<$length; $i++) {
        ...
        $aside.children[$i].addEventListener('click', 
            function($e) {
                $e.stopPropagation();
                clearInterval($interval);
                $previousPosition = $position;
                $direction = ($e.target.getBoundingClientRect().left + $e.target.clientWidth/2 ) > $e.clientX  ? "<" : ">";
                startSlide(true);
        ...
        $aside.children[$i].addEventListener('mouseover', 
            function($e) {
                $e.stopPropagation();
                clearInterval($interval); 
            }
        ...
        $aside.children[$i].addEventListener('mouseout', 
            function($e) {
                $e.stopPropagation();
                clearInterval($interval);
                startSlide();
